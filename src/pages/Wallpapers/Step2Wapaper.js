...
const Step2Wallpaper = () => {
  ...
  const handleClickContinue = async () => {
    const newFinishedStep = { ...finished, [STEP_OPTIONS[2]]: true };
    navigate("/map#step2", { state: { finished: newFinishedStep } });
  };

  const handleClickClose = () => {
    navigate("/map#step2", { state: { finished } });
  };

  const handleDownload = async () => {
    downloadImage(IMAGES.STEP2_WALLPAPER, "holiday-wallpaper.png");
  };

  const handleLongPress = async () => {
    downloadImage(IMAGES.STEP2_WALLPAPER, "journey-to-the-peak");
  };
  ...
  return (
    <Box sx={modalStlye.wallpaperContentSx} className="map-page">
      <div className="bg-container">
        <CustomImage src={IMAGES.STEP2_BG} alt="map" className="bg-img" />
      </div>
      ...
      >
        <Box sx={modalStlye.modalWallpaperContentSx}>
          <CustomImage
            src={IMAGES.STEP2_WALLPAPER_CONTENT}
            alt="modal-content"
            className={`modal-content step1-wallpaper-content`}
          />
          <CustomImage
            src={IMAGES.STEP2_WALLPAPER}
            alt="modal-content"
